---
created: 2026-09-15
updated: 2026-09-15
tipo: rutina + progreso
objetivo: volumen muscular
frecuencia: Lun/Mar/Jue/Vie (~2h por sesión)
---

# 🏋️ GYM — Volumen 4x

> **Regla de oro:** 2 frecuencias por músculo + sobrecarga progresiva. Técnica > peso. RIR 1-2 en todo.

## 📐 Volumen semanal objetivo (series efectivas)

| Músculo | Series/sem | Reparto |
|---|---|---|
| Pecho | 14 | 7 + 7 |
| Dorsal | 14 | 7 + 7 |
| Deltoide lateral | 14 | 7 + 7 |
| Deltoide posterior | 12 | 6 + 6 |
| Bíceps | 12 | 6 + 6 |
| Tríceps | 12 | 6 + 6 |
| Cuádriceps | 14 | 8 + 6 |
| Femoral | 12 | 5 + 7 |
| Glúteo | 14 | 6 + 8 |
| Pantorrilla | 14 | 7 + 7 |
| Abdomen | 10 | 5 + 5 |

---

## LUN — Torso A (empuje foco pecho + jalón ancho)

| # | Ejercicio | Músculo | Series x Reps | Descanso | Notas |
|---|---|---|---|---|---|
| 1 | Press banca barra o mancuernas | Pecho | 4 x 6-8 | 2-3 min | Principal. Progresión lineal |
| 2 | Dominadas / Jalón al pecho | Dorsal | 4 x 6-10 | 2 min | Si no salen dominadas, jalón pesado |
| 3 | Press inclinado mancuerna | Pecho superior | 3 x 8-10 | 2 min | Pecho superior |
| 4 | Remo con barra o mancuerna | Espalda alta + dorsal | 3 x 8-10 | 90 seg | Torso ~45°, apretar 1 seg |
| 5 | Elevaciones laterales | Deltoide lateral | 3 x 12-15 | 60-90 seg | Sin impulso, pausa arriba |
| 6 | Curl barra + Fondos (superset) | Bíceps + tríceps | 3 x 8-12 c/u | 90 seg | Brazo completo |

**Total: 20 series**

---

## MAR — Pierna Full A (foco cuádriceps)

| # | Ejercicio | Músculo | Series x Reps | Descanso | Notas |
|---|---|---|---|---|---|
| 1 | Sentadilla barra o hack | Cuádriceps + glúteo | 4 x 6-8 | 2-3 min | Principal. Profundidad completa |
| 2 | Pull-through en polea (bisagra guiada) | Femoral + glúteo | 3 x 12-15 | 90 seg | Reemplaza RDL temporal. Aprende bisagra sin carga lumbar. Progresión a RDL en 3-4 sem |
| 3 | Prensa / Leg press | Cuádriceps | 3 x 10-12 | 2 min | Pies medio-bajos, quads |
| 4 | Hip thrust | Glúteo | 3 x 10-12 | 90 seg | Pausa 2 seg arriba |
| 5 | Curl femoral tumbado/sentado | Femoral | 3 x 10-12 | 90 seg | |
| 6 | Pantorrilla de pie + Abs (superset) | Pantorrilla + abdomen | 4 x 12-15 c/u | 60 seg | Pausa arriba y abajo |

**Total: 20 series**

---

## JUE — Torso B (empuje foco hombro + jalón grosor)

| # | Ejercicio | Músculo | Series x Reps | Descanso | Notas |
|---|---|---|---|---|---|
| 1 | Press militar barra o mancuerna | Deltoide frontal + lateral | 4 x 6-8 | 2-3 min | Principal del día |
| 2 | Remo T o remo polea agarre neutro | Espalda grosor + dorsal | 4 x 8-10 | 2 min | Grosor espalda |
| 3 | Press banca o fondos pesados | Pecho + tríceps | 3 x 6-10 | 2 min | Pecho, variante del lunes |
| 4 | Jalón agarre supino / Remo 1 brazo | Dorsal bajo + bíceps | 3 x 8-12 | 90 seg | Dorsal bajo |
| 5 | Elevaciones laterales + Pájaros (superset) | Lateral + posterior | 3 x 12-15 c/u | 60-90 seg | Hombro completo |
| 6 | Press francés + Curl martillo (superset) | Tríceps + bíceps | 3 x 10-12 c/u | 90 seg | Brazo completo |

**Total: 20 series**

---

## VIE — Pierna Full B (foco femoral + glúteo)

| # | Ejercicio | Músculo | Series x Reps | Descanso | Notas |
|---|---|---|---|---|---|
| 1 | Hip thrust pesado (principal) | Glúteo + femoral | 4 x 6-8 | 2-3 min | Sube a principal mientras RDL en pausa. Pausa 2 seg arriba |
| 2 | Sentadilla búlgara o hack | Cuádriceps + glúteo | 3 x 8-10 | 2 min | Quad con menos carga axial |
| 3 | Hiperextensión en banco | Glúteo + femoral | 3 x 12-15 | 90 seg | Bisagra sin carga, alternativa suave al RDL |
| 4 | Curl femoral | Femoral | 3 x 10-12 | 90 seg | |
| 5 | Desplante caminando o estático | Glúteo + cuádriceps | 3 x 10-12 c/pierna | 90 seg | Glúteo + quad |
| 6 | Pantorrilla sentado + Crunch polea (superset) | Pantorrilla + abdomen | 3 x 12-15 c/u | 60 seg | Sóleo + abdomen |

**Total: 20 series**

---

## 📈 Reglas de progresión (leer cada semana)

1. **Doble progresión:** cuando complete el techo de reps en todas las series con RIR 1-2, suba peso (2.5 kg torso / 5 kg pierna) y vuelva al piso del rango.
2. **Arranque conservador:** semanas 1-2 al 80% del peso que cree manejar. La semana 3 ya va al 100%.
3. **Deload cada 6-8 semanas:** 1 semana al 60% volumen. Sin deload no hay supercompensación.
4. **Si falla 2 semanas seguidas en el mismo ejercicio:** baje 10% el peso y reconstruya.
5. **Descansos reales:** con 2h disponibles, descanse lo indicado. Recortar descanso ≠ más hipertrofia.

---

## 📏 Registro de progreso

> Medir mismo día, misma hora, ayunas. Fotos misma luz y pose.

### Semana 0 — Base (15/09/2026)

| Medida | Valor |
|---|---|
| Peso corporal | _pendiente_ |
| Pecho | _pendiente_ |
| Brazo relajado / flex | _pendiente_ |
| Cintura (ombligo) | _pendiente_ |
| Cadera | _pendiente_ |
| Muslo medio | _pendiente_ |
| Pantorrilla | _pendiente_ |

**Fotos base:** _pendiente (frente / lateral / espalda)_

### Fuerza base (pesos semana 0)

> Sesión 15/09 (Mar) — excepción: se hizo Torso A en lugar de Pierna A. Solo por hoy, el plan Lun/Mar/Jue/Vie sigue igual.

| Ejercicio | Peso x Reps |
|---|---|
| Press pecho mancuerna (15/09) | 10 kg c/lado x 8 |
| Jalón / Espalda (15/09) | 40 kg x 8 |
| Curl bíceps (15/09) | 10 kg x 8 |
| Press hombro mancuerna (15/09) | 7.5 kg c/lado x 8 |
| Tríceps polea desmultiplicadora (15/09) | 15 kg x 8 |
| Sentadilla | _pendiente (Pierna A)_ |
| RDL | _pendiente (Pierna A)_ |
| Press banca barra | _pendiente_ |
| Prensa (16/09) | 60 kg x 8 (4 series) |
| Extensión cuádriceps (16/09) | 60 kg x 8 (4 series) |
| Hip thrust (16/09) | 20 kg x 8 (4 series) |
| Femoral sentado (16/09) | 40 kg x 8 (4 series) |
| Abductores (16/09) | 30 kg x 8 (4 series) |

### Bitácora semanal

| Semana | Peso | Pecho | Brazo | Cintura | Muslo | PR destacado | Notas |
|---|---|---|---|---|---|---|---|
| 0 (15/09) | | | | | | Torso A hecho (excepción): espalda 40kg x8, pecho 10kg/lado x8, bíceps 10kg x8, hombro 7.5kg/lado x8, tríceps polea 15kg x8 | Base |
| 1 | | | | | | | |
| 2 | | | | | | | |
| 4 | | | | | | | Revisión mensual |
| 8 | | | | | | | Revisión + deload |

---

## ✅ Checklist sesión

- [ ] Calentamiento 5-8 min (movilidad + 2 series aproximación en el principal)
- [ ] Agua + creatina noche 3-5g (no pre-entreno, es acumulación)
- [ ] Anotar peso x reps de cada ejercicio en bitácora
- [ ] Proteína día: ~2g x kg corporal (volumen sin proteína = estancamiento)

---

`🦾 H.E.L.E.N. — Si está aquí, está priorizado.`
